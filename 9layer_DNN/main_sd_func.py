...
def AE5_sd(div_pre, E, train_data, test_data, dim, H1, H2):
    # os.chdir('/Users/itoukeisuke/Python/pycharm/5layer_AESD/program/')
    collect = False
    check = False
    # Make Data
    if div_pre != "":
    ...
    for k in range(div):
        ...
        print('|          SDAE 1st Training         |')
        for epoch in range(EPOCH):
            if (epoch % PARAMETER['MOD_EPOCH']) == 0:
                print('|[SDAE1]   --- EPOCH: {} / {} ---   |'.format(epoch+1, EPOCH))
            ...
            if (epoch % PARAMETER['MOD_EPOCH']) == 0:
                ...
                print('|[SDAE1] Epoch: {} / {} | Error: {:.2f} |'.format(epoch+1, EPOCH, e_1))

        print('|        SDAE1 Last Error: {:.2f}       |'.format(errmin1))


        # 2層目(hidden1 -> hidden2)
        print('|          SDAE 2nd Training         |')
        ...
        for epoch in range(EPOCH):
            if (epoch % PARAMETER['MOD_EPOCH']) == 0:
                print('|[SDAE2]   --- EPOCH: {} / {} ---   |'.format(epoch+1, EPOCH))
            ...
            if (epoch % PARAMETER['MOD_EPOCH']) == 0:
                ...
                print('|[SDAE2] Epoch: {} / {}, Error: {:.2f} |'.format(epoch+1, EPOCH, e_2))

        print('|       SDAE2 Last Error: {:.2f}        |'.format(errmin2))
        ...
        for epoch in range(EPOCH):
            ...
            if (epoch % PARAMETER['MOD_EPOCH']) == 0:
                ...
                print('|[SDNN5] Epoch: {} / {} | Error: {:.2f} |'.format(epoch+1, EPOCH, e_5))

        print('|       SDNN5 Last Error: {:.2f}        |'.format(errmin5))
        print("")
        print("### --- ALL SD TRAINING FINISH --- ###")
        print("")
    ...
    # plt.plot(epochs, Error_decay, label='Ks: 60, Ds: 30')
