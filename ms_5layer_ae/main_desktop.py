...
def ms_desktop(E, h1, h2):
    # os.chdir('/Users/itoukeisuke/Python/pycharm/5layer_AEMS/program/')
    # os.chdir('/Users/itoukeisuke/Python/pycharm/program_ms/')
    collect = True
    check = True

    # Make Data
    div = 6
    X_train, Y_train, dim = MakeDataMS(div)
    ...
    plt.plot(epochs, Error_decay, label='$K_{M}$: 60, $D_{M}$: 30')
    plt.title('Error Reduction')
    plt.xlabel('Epoch')
    plt.ylabel('Reconstruction Error')
    plt.legend()
    plt.show()
    plt.savefig('Error_Reduction_MS.png')
